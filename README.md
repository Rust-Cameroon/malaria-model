...
### Generate Crops + Manifest (MP-IDB + Uninfected)

This project trains from a CSV manifest generated by `mpidb_prep`.

Expected input folders:

1) **MP-IDB species dataset** (infected + stage labels) (one folder per species):
...
2) **Uninfected negatives** (for the "not infected" gate):
...
Generate crops (one crop per parasite blob in the mask) + a `manifest.csv`:
...
- `infected` is `1` for MP-IDB parasite crops and `0` for uninfected samples.
- Stage labels are inferred from the filename stem tokens (`R`, `T`, `S`, `G`).
- Train/valid split is done by `source_image_id` to avoid leakage (crops from the same source image do not get split across train/valid).
...
This will train a multi-class model with:

- `Falciparum`, `Malariae`, `Ovale`, `Vivax`, and an internal `Uninfected` class used for gating.
- A second output head predicts stage presence flags (`R/T/S/G`).

Training uses the manifest path configured in `src/config.rs`:
...
- If the predicted class is `Uninfected`, treat the cell as **not infected**.
- Otherwise, the predicted class corresponds to the detected species.
- Stage predictions are only meaningful when infected.
